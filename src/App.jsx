...
function normalizeData(d){
  ...
  if(!d.lustrumLedger)d.lustrumLedger={};
  if(!d.expenses)d.expenses=[];
  if(!d.income)d.income={};
  if(!d.categories)d.categories=[];
  if(!d.members)d.members=[];
  d.members.forEach(m=>{if(m.lustrumStart==null)m.lustrumStart=0;});
...
function Overzicht({data,go}){
  ...
  const lustrumTot=data.members.reduce((a,m)=>a+lustrumTotal(data,m.id),0);const lustrumDoel=data.members.reduce((a,m)=>a+(m.target||0),0);
...
// Totaal gespaard = beginsaldo (vóór maandelijkse tracking) + som van de maandelijkse inleg.
...
function Lustrum({data,update,edit}){
  ...
  const rows=[...data.members].sort((a,b)=>lustrumTotal(data,b.id)-lustrumTotal(data,a.id));
  const target=data.lustrumTarget||0;
  const tot=data.members.reduce((a,m)=>a+lustrumTotal(data,m.id),0);const doel=data.members.length*target;
  ...
  const perLid=rows.map(m=>{
    const total=lustrumTotal(data,m.id);
    const nogTeSparen=Math.max(0,target-total);
    ...
    return{m,total,nogTeSparen,doelBereikt,perMaand,status};
  });
  ...
  return (<div className="jc-grid">
      ...
      <div className="jc-luslist">{perLid.map(({m,total,nogTeSparen,doelBereikt,perMaand,status})=>{const pct=target?Math.min(100,(total/target)*100):0;
        return(<div key={m.id} className="jc-lusrow" onClick={()=>setSel(m.id)} style={{cursor:"pointer"}}><span className="jc-av sm" style={{background:m.color}}>{m.short[0]}</span>
          <div className="jc-lusmid">
            <div className="jc-lustop"><span>{m.short}</span><span className="ochre">{eur0(total)}</span></div>
    ...
    {sel&&<LustrumSheet data={data} update={update} mid={sel} edit={edit} onClose={()=>setSel(null)}/>}
  </div>);
}
...
function Beheer({data,update,setData,markDirty}){
  ...
  const addMember=()=>update(d=>{d.members.push({id:"m"+Date.now(),name:"Nieuw lid",short:"Nieuw",type:"fulltime",rate:270,color:"#9c6b3f",lustrumStart:0,target:d.lustrumTarget||3000});});
  ...
  const reset=()=>{const n=normalizeData(structuredClone(SEED));n.lastUpdated=TODAY;setData(n);markDirty?.();setConfirmReset(false);};
  ...
  return (<div className="jc-grid">
    <div className="jc-card"><h3 className="jc-h">Algemeen</h3>
      ...
      <Field label="Pincode penningmeester"><input value={data.pmPin} onChange={e=>setField("pmPin",e.target.value)}/></Field>
    </div>
