...
// ───────────── storage (lokaal, per browser — vervangt window.storage uit de artifact-omgeving) ─────────────
const KEY = "jaarclub_kas_v3";
const store = {
  async get(){ try{ const r=localStorage.getItem(KEY); return r?JSON.parse(r):null; }catch(e){ return null; } },
  async set(data){ try{ localStorage.setItem(KEY, JSON.stringify(data)); }catch(e){ console.error(e); } },
};
...
export default function App(){
  ...
  useEffect(()=>{(async()=>{const s=await store.get();const n=s?structuredClone(s):structuredClone(SEED);backfillHistoricalDates(n);mergeBorrelDixo(n);migrateForecast(n);setData(n);loaded.current=true;})();},[]);
  useEffect(()=>{if(!loaded.current||!data)return;setSaving(true);const t=setTimeout(async()=>{await store.set(data);setSaving(false);},400);return()=>clearTimeout(t);},[data]);
